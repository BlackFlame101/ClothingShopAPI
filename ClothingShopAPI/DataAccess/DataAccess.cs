...
namespace ClothingShopAPI.DataAccess
{
    public class DataAccess : IDataAccess
    {
        ...
        public bool DeleteCartItem(int userId, int itemId)
        {
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                connection.Open();
                string query = $"DELETE FROM CartItems WHERE CartId IN (SELECT CartId FROM Carts WHERE UserId = {userId} AND Ordered = 'false') AND ItemId = {itemId};";
            ...
            using (SqlConnection connection = new SqlConnection(dbconnection))
            {
                ...
                while (reader.Read())
                {
                    Order order = new Order
                    {
                        ...
                        Payment = GetPayments(new Payment { Id = (int)reader["PaymentId"] }),
        ...
        public ItemCategory GetItemCategory(int id)
        {
            ...
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                while (r.Read())
                {
                    ...
                    ItemCategory.SubCategory = (string)r["SubCategory"];
                }
            }
        ...
        public List<Review> GetItemReviews(int ItemId)
        {
            ...
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                string query = "SELECT * FROM Reviews WHERE ItemId=" + ItemId + ";";
                command.CommandText = query;
        ...
        public List<Item> GetItems(string category, string subcategory, int count)
        {
            ...
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                string query = "SELECT TOP " + count + " * FROM Items WHERE CategoryId=(SELECT CategoryId FROM ItemCategories WHERE Category=@c AND SubCategory=@s) ORDER BY newid();";
                command.CommandText = query;
        ...
        public Payment GetPayments(Payment payment)
        {
            using (SqlConnection connection = new SqlConnection(dbconnection))
            {
                SqlCommand command = new SqlCommand()
                ...
                if (reader.Read())
                {
                    ...
                    payment.User = GetUser((int)reader["UserId"]);
        ...
        public void InsertNewItem(Item item)
        {
            using (SqlConnection connection = new SqlConnection(dbconnection))
            {
                ...
                string query = "INSERT INTO Items (Title, Description, Price, Quantity, ImageName, CategoryId, OfferId) " +
                ...
                command.Parameters.AddWithValue("@CategoryId", item.ItemCategory.Id); 
                command.Parameters.AddWithValue("@OfferId", item.Offer.Id); 
        ...
        public void InsertReview(Review review)
        {
            ...
            string query = "INSERT INTO Reviews (UserId, ItemId, Review, CreatedAt) VALUES (@uid, @pid, @rv, @cat);";
            command.CommandText = query;
            command.Parameters.Add("@uid", System.Data.SqlDbType.Int).Value = review.User.Id;
            command.Parameters.Add("@pid", System.Data.SqlDbType.Int).Value = review.Item.Id;
        ...
        public bool InsertUser(User user)
        {
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                query = "INSERT INTO Users (FirstName, LastName, Address, Mobile, Email, Password, CreatedAt, ModifiedAt, UserType) values (@fn, @ln, @add, @mb, @em, @pwd, @cat, @mat, @type);";
                ...
                command.Parameters.Add("@type", System.Data.SqlDbType.NVarChar).Value = user.UserType;
        ...
        public string IsUserPresent(string email, string password)
        {
            ...
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                string key = "c2f5a1e8b4d7a9f0c3b6e2d9a4c8d5b2";
                string duration = "60";
